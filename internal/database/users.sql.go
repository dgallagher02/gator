...
) SELECT inserted_posts.id, inserted_posts.created_at, inserted_posts.updated_at, inserted_posts.title, inserted_posts.url, inserted_posts.description, inserted_posts.published_at, inserted_posts.feed_id
FROM inserted_posts
...
func (q *Queries) Unfollow(ctx context.Context, arg UnfollowParams) (FeedFollow, error) {
	row := q.db.QueryRowContext(ctx, unfollow, arg.Name, arg.Url)
	var i FeedFollow
	err := row.Scan(
		&i.ID,
		&i.CreatedAt,
